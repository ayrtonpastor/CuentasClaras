...
class Vista_lista_actividades(QWidget):
    ...
    def inicializar_GUI(self):
        ...
        self.distribuidor_base.addWidget(
            self.etiqueta_bienvenida, Qt.AlignCenter)

        self.etiqueta_descripcion = QLabel(
            "La unica aplicacion que te permite organizar actividades con tus amigos de forma justa")
        self.etiqueta_descripcion.setAlignment(Qt.AlignCenter)
        self.distribuidor_base.addWidget(
            self.etiqueta_descripcion, Qt.AlignCenter)
        ...
        self.distribuidor_botones.addWidget(
            self.btn_aniadir_actividad, 0, 0, Qt.AlignLeft)
        self.btn_aniadir_actividad.clicked.connect(
            self.mostrar_dialogo_insertar_actividad)
        ...
        self.distribuidor_botones.addWidget(
            self.btn_ver_viajeros, 0, 1, Qt.AlignRight)
        ...
        self.widget_tabla_actividades.setLayout(
            self.distribuidor_tabla_actividades)
        ...
        if (self.actividades != None and len(self.actividades) > 0):
            ...
            for my_actividad in self.actividades:
                ...
                btn_ver_actividad.setIcon(
                    QIcon("src/recursos/002-eye-variant-with-enlarged-pupil.png"))
                btn_ver_actividad.clicked.connect(
                    partial(self.mostrar_actividad, my_actividad))
                self.distribuidor_tabla_actividades.addWidget(
                ...
                btn_ver_viajeros.clicked.connect(
                    partial(self.mostrar_dialogo_insertar_viajeros, my_actividad))
                self.distribuidor_tabla_actividades.addWidget(
                ...
                btn_eliminar.clicked.connect(
                    partial(self.eliminar_actividad, my_actividad.id))
                self.distribuidor_tabla_actividades.addWidget(
    ...
    def mostrar_dialogo_insertar_viajeros(self, actividad):
        ...
        if dialogo.resultado == 1:
            self.interfaz.actualizar_viajeros(
                actividad, dialogo.nuevos_viajeros, dialogo.eliminar_viajeros)
    ...
    def eliminar_actividad(self, actividad_id):
        ...
        mensaje_confirmacion.setText(
            "¿Esta seguro de que desea borrar esta actividad?\nRecuerde que esta acción es irreversible")
        ...
        mensaje_confirmacion.setStandardButtons(
            QMessageBox.Yes | QMessageBox.No)
        respuesta = mensaje_confirmacion.exec_()
        if respuesta == QMessageBox.Yes:
            self.interfaz.eliminar_actividad(actividad_id)
